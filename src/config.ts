...
export function openConfigPanel(): void {
  ...
  const downloadCheck = makeCheckbox(
    'Show "Download" button',
    settings.showDownload,
  );
  const obsidianCheck = makeCheckbox(
    'Show "Save to Obsidian" button',
    settings.showObsidian,
  );

  const vaultField = makeField(
    "Obsidian vault name",
    "e.g. My Vault",
    settings.obsidianVault,
  );
  const folderField = makeField(
    "Folder path (blank = vault root)",
    "e.g. Clippings",
    settings.obsidianFolder,
  );

  const debugCheck = makeCheckbox(
    "Enable debug capture (logs every API response)",
    settings.debug,
  );
